...
function DateRangePicker({ startDate, endDate, onApply, onClose }: {
  ...
  function preset(days: number) {
    ...
    setS(start.toISOString().split('T')[0]); setE(end.toISOString().split('T')[0])
  }
    ...
    <div style={{ position:'absolute', top:'100%', right:0, zIndex:500, marginTop:'6px', background:C.s, border:`1px solid ${C.b}`, borderRadius:'14px', padding:'16px', boxShadow:'0 20px 60px rgba(0,0,0,0.6)', width:'300px' }}>
      <div style={{ display:'flex', gap:'6px', flexWrap:'wrap', marginBottom:'14px', paddingBottom:'14px', borderBottom:`1px solid ${C.b}` }}>
        {[['7d', ()=>preset(7)], ['30d', ()=>preset(30)], ['90d', ()=>preset(90)], ['This month', presetMonth], ['Last month', presetLastMonth]].map(([label, fn]: any) => (
          <button key={label} onClick={fn} style={{ padding:'4px 10px', borderRadius:'6px', fontSize:'11px', fontWeight:600, cursor:'pointer', background:C.s3, border:`1px solid ${C.b}`, color:C.mid }}>{label}</button>
        ))}
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:'2px' }}>
        {cells.map((day, i) => {
          ...
          return (
            <button key={i} onClick={() => handleDay(day)} style={{ padding:'6px 0', borderRadius:'6px', border:'none', cursor:'pointer', fontSize:'12px', fontWeight: start||end ? 700 : 400, background: start||end ? C.cyan : inRange ? 'rgba(0,200,224,0.15)' : 'transparent', color: start||end ? '#080c0f' : inRange ? C.cyan : today ? C.cyan : C.txt, outline: today&&!start&&!end ? `1px solid ${C.b}` : 'none' }}>
              {day}
            </button>
          )
        })}
      ...
      <div style={{ display:'flex', gap:'8px', marginTop:'12px' }}>
        ...
        <button onClick={() => { if (s&&e) onApply(s,e) }} disabled={!s||!e} style={{ flex:1, padding:'8px', borderRadius:'8px', fontSize:'13px', fontWeight:700, cursor:'pointer', background:s&&e?C.cyan:C.s3, color:s&&e?'#080c0f':C.muted, border:'none' }}>Apply</button>
      </div>
    </div>
  )
}
...
export default function DashboardClient({
  ...
  useEffect(() => {
    ...
    async function refetch() {
      ...
      if (period === 'custom' && customStart && customEnd) {
        ...
        s = startDate.toISOString().split('T')[0]; e = endDate.toISOString().split('T')[0]
      }
  ...
  useEffect(() => {
    ...
    async function fetchPrev() {
      ...
      const now = new Date()
      if (period === 'today') {
        pEnd = new Date(now); pEnd.setHours(0,0,0,0); pEnd.setSeconds(-1)
        pStart = new Date(pEnd); pStart.setDate(pStart.getDate()-1)
      } else if (period === 'week') {
        pEnd = new Date(now); pEnd.setDate(pEnd.getDate()-7)
      ...
      } else { setPrevTotals(null); return }
      const s = pStart.toISOString().split('T')[0], e = pEnd.toISOString().split('T')[0]
  ...
  function getDelta(key: string): { pct: number; up: boolean } | null {
    ...
    const cur = allTotals[key] || 0, prev = prevTotals[key] || 0
  ...
  function setParam(key: string, val: string) {
    const p = new URLSearchParams(searchParams.toString()); p.set(key, val)
  ...
  function applyCustomRange(s: string, e: string) {
    ...
    router.push(`/dashboard?${p.toString()}`); setShowDatePicker(false)
  }
  ...
  campaigns.forEach((cm: any) => {
    ...
    campaignMap[k].spend += Number(cm.spend||0); campaignMap[k].conversions += Number(cm.conversions||0)
    campaignMap[k].leads += Number(cm.leads||0); campaignMap[k].conversion_value += Number(cm.conversion_value||0)
  })
  ...
  function getRaw(key: string, val: number) { return key === 'roas' ? Math.round(val*100) : Math.round(val) }
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      ...
      <div style={{ background:'var(--surface)', borderBottom:'1px solid var(--border)', padding:'0 16px', height:'64px', display:'flex', alignItems:'center', gap:'6px', flexShrink:0, minWidth:0 }}>
        <div style={{ flex:'1 1 0', minWidth:0, overflow:'hidden' }}>
          <div style={{ fontFamily:'Syne, sans-serif', fontSize:'18px', fontWeight:700, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{clientData?.name||'Dashboard'}</div>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'6px', flexShrink:0, flexWrap:'wrap', justifyContent:'flex-end' }}>
          ...
          <div ref={datePickerRef} style={{ position:'relative', flexShrink:0 }}>
            ...
            {showDatePicker && (
              <DateRangePicker startDate={customStart} endDate={customEnd} onApply={applyCustomRange} onClose={() => setShowDatePicker(false)} />
            )}
          </div>
          <button onClick={() => setShowMetricPicker(v => !v)}
            style={{ padding:'6px 10px', borderRadius:'8px', fontSize:'12px', fontWeight:600, cursor:'pointer', background:showMetricPicker?'var(--cyan)':'transparent', border:'1px solid var(--border)', color:showMetricPicker?'var(--black)':'var(--text-muted)', whiteSpace:'nowrap', flexShrink:0 }}>🎛</button>
          <button onClick={exportCSV}
            style={{ padding:'6px 10px', borderRadius:'8px', fontSize:'12px', fontWeight:600, cursor:'pointer', background:'transparent', border:'1px solid var(--border)', color:'var(--text-muted)', whiteSpace:'nowrap', flexShrink:0 }}>⬇</button>
        </div>
      </div>
            ...
            {visibleMetrics.map((m, i) => {
              ...
              return (
                <KpiCard key={m.key} label={m.label} accent={m.accent}
                  rawValue={getRaw(m.key, val)} displayValue={doFmt(val, m.fmt)}
                  sub={getSub(m.key)} delta={getDelta(m.key)}
                  dragging={dragFrom===i}
                  onDragStart={() => setDragFrom(i)} onDragEnter={() => setDragTo(i)} onDragEnd={handleDragEnd}
                />
              )
            })}
          </div>
