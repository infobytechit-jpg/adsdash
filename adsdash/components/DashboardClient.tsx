    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
          {accounts.length > 0 && (
            <select value={selectedAccount} onChange={e => setParam('account', e.target.value)}
              style={{ fontSize: '12px', padding: '6px 8px', background: 'var(--surface2)', border: '1px solid var(--border)', color: 'var(--text)', borderRadius: '8px', outline: 'none', maxWidth: '130px' }}>
          ...
          <div style={{ display: 'flex', background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: '8px', overflow: 'hidden', flexShrink: 0 }}>
            {([['today', 'Today'], ['week', 'Week'], ['month', 'Month'], ['all', 'All']] as const).map(([p, label]) => (
              <button key={p} onClick={() => setParam('period', p)}
                style={{ padding: '6px 10px', fontSize: '12px', fontWeight: 600, border: 'none', cursor: 'pointer', background: period === p ? 'var(--cyan)' : 'transparent', color: period === p ? 'var(--black)' : 'var(--text-muted)', whiteSpace: 'nowrap' }}>
                {label}
              </button>
            ))}
          </div>
      ...
      {showMetricPicker && (
        <div style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)', padding: '12px 28px', flexShrink: 0 }}>
          <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginBottom: '10px', fontWeight: 600 }}>TOGGLE METRICS · Changes saved automatically · Drag cards to reorder</div>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '24px' }}>
          {[{ key: 'all', label: 'All Platforms', color: '#00C8E0' }, { key: 'google', label: 'Google Ads', color: '#4285F4' }, { key: 'meta', label: 'Meta Ads', color: '#1877F2' }].map(p => (
            <button key={p.key} onClick={() => setParam('platform', p.key)}
              style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 16px', borderRadius: '100px', fontSize: '13px', fontWeight: 600, cursor: 'pointer', border: `1px solid ${platform === p.key ? p.color : 'var(--border)'}`, background: platform === p.key ? `${p.color}20` : 'var(--surface2)', color: platform === p.key ? p.color : 'var(--text-muted)' }}>
